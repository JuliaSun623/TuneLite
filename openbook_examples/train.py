...
from openbook_examples.myarguments import ModelArguments, DataArguments, MyTuneLiteArguments
from openbook_examples.mycollator import DataCollatorForCauselLM, EvalDataCollatorForCauselLM
from openbook_examples.mydatasets import MyDataset, get_dataset_info
...
def train():
    ...
    tag_name = '_'.join(
        [data_args.dataset_name, model_name, tl_args.tag] if tl_args.tag else [data_args.dataset_name, model_name])
    hparam_name = 'output'
    ...
    wandb_config = copy.deepcopy(asdict(tl_args))
